...
function getLanguage() {
  try {
    return localStorage.getItem('tmdbLang') || 'en-US';
  } catch {
    return 'en-US';
  }
}
    ...
    episodes.forEach((ep) => {
      const card = createElement(`
        <div class="cursor-pointer group relative">
          ...
          <p class="mt-1 text-xs truncate text-gray-200">${ep.episode_number}. ${ep.name}</p>
        </div>
      `);
      card.onclick = () => {
...
function renderCard(item, type) {
  ...
  const el = createElement(`
    <div class="relative overflow-hidden rounded-lg shadow-lg group cursor-pointer transition-transform duration-300 hover:scale-105 bg-gray-900">
      ...
      <button class="absolute top-2 right-2 bg-black/70 hover:bg-black text-white rounded-full p-1 watchlist-btn" title="${inList ? 'Remove from watchlist' : 'Add to watchlist'}"
  ...
  const el = createElement(`
    <div class="relative flex-shrink-0 w-64 sm:w-72 md:w-80 lg:w-96 cursor-pointer group">
      ...
      <div class="absolute bottom-2 left-2 right-2 flex justify-between items-center">
        <span class="text-white text-sm font-semibold truncate pr-2">${title}</span>
      </div>
    </div>
  `);
...
function renderPlatformSlider(container) {
  ...
  PLATFORMS.forEach((platform) => {
    ...
    card.className = 'flex-shrink-0 w-36 h-20 bg-gray-800 rounded-lg flex items-center justify-center text-sm font-semibold mr-4';
    card.textContent = platform.name;
    container.appendChild(card);
  });
}
...
function renderRatingRing(voteAvg) {
  ...
  return `
    <div class="absolute top-2 left-2 w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold text-white"
      style="background: conic-gradient(${color} 0deg ${deg}deg, #2d2d2d ${deg}deg 360deg);">
      <span class="absolute inset-0 flex items-center justify-center">${voteAvg}</span>
    </div>
  `;
}
...
async function initMoviesPage() {
  const hero = document.getElementById('hero');
  const movieSection = document.getElementById('movies-popular');
  const topSection = document.getElementById('movies-top-rated');
  const genreContainer = document.getElementById('movies-genres');
  // Show skeleton placeholders while loading
  showSkeletons(movieSection, 12);
  showSkeletons(topSection, 12);
  // Fetch popular and top rated in parallel
  const [movies, topRated, genres] = await Promise.all([
    getPopular('movie'),
    getTopRated('movie'),
    getGenres('movie'),
  ]);
  ...
  if (movies.length) {
    renderHero(hero, movies[0], 'movie');
  }
  // Render genre chips
  if (genreContainer) {
    ...
    allBtn.className = 'px-3 py-1 rounded-full border border-gray-600 text-sm whitespace-nowrap bg-red-600 text-white';
    allBtn.onclick = () => {
      ...
      // Render original popular and top rated grids
      renderGrid(movieSection, movies.slice(1, 13), 'movie');
    };
    genreContainer.appendChild(allBtn);
    genres.forEach((g) => {
      const btn = document.createElement('button');
      btn.textContent = g.name;
      btn.className = 'px-3 py-1 rounded-full border border-gray-600 text-sm whitespace-nowrap bg-gray-800 text-gray-300 hover:bg-gray-700';
      btn.onclick = async () => {
        // Update active state
        genreContainer.querySelectorAll('button').forEach((b) => {
          b.classList.remove('bg-red-600', 'text-white');
        ...
        showSkeletons(movieSection, 12);
        try {
          const results = await discoverContent('movie', { with_genres: g.id });
          if (results.length) {
            // Use first item as hero
            renderHero(hero, results[0], 'movie');
            renderGrid(movieSection, results.slice(1, 13), 'movie');
          } else {
            movieSection.innerHTML = '<p class="text-gray-400">No movies found.</p>';
          }
        } catch (err) {
          console.error(err);
        }
  ...
  renderGrid(movieSection, movies.slice(1, 13), 'movie');
  renderGrid(topSection, topRated.slice(0, 12), 'movie');

  // Infinite scroll for the movies grid
  const moviesState = {
    page: 2, // we already rendered page 1 (slice 1..13)
    loading: false,
    noMore: false,
    genreId: null,
  };
  // Load more movies when reaching near the bottom of the page
  async function loadMoreMovies() {
    if (moviesState.loading || moviesState.noMore) return;
    moviesState.loading = true;
    try {
      let results = [];
      if (moviesState.genreId) {
        // Fetch next page of movies for the selected genre
        const lang = getLanguage();
        const params = new URLSearchParams({ api_key: API_KEY, language: lang, page: moviesState.page, with_genres: moviesState.genreId });
        const data = await fetchJSON(`${API_BASE}/discover/movie?${params}`);
        results = data.results || [];
        // Stop if no more pages
        if (data.total_pages && moviesState.page >= data.total_pages) {
          moviesState.noMore = true;
        }
      } else {
        results = await getPopular('movie', moviesState.page);
        // Popular endpoint does not return total pages; we assume there are pages up to 500 as per TMDB docs
        if (!results.length) {
          moviesState.noMore = true;
        }
      }
      if (results.length) {
        appendGrid(movieSection, results, 'movie');
        moviesState.page += 1;
      } else {
        moviesState.noMore = true;
      }
  ...
  // Update infinite scroll state when genre filter changes
  if (genreContainer) {
    genreContainer.querySelectorAll('button').forEach((btn) => {
      btn.addEventListener('click', () => {
        // Determine selected genre after click.  "All" resets the genre.
        const genreButtons = Array.from(genreContainer.querySelectorAll('button'));
        const active = genreButtons.find((b) => b.classList.contains('bg-red-600'));
        if (active && active.textContent !== 'All') {
          // Use the genres array fetched earlier to map name to id
          const genreObj = genres.find((g) => g.name === active.textContent);
          moviesState.genreId = genreObj ? genreObj.id : null;
        } else {
          moviesState.genreId = null;
        }
        moviesState.page = 2;
        moviesState.noMore = false;
      });
    });
  }
}
...
async function initTvPage() {
  const hero = document.getElementById('hero');
  const tvSection = document.getElementById('tv-popular');
  const topSection = document.getElementById('tv-top-rated');
  const genreContainer = document.getElementById('tv-genres');
  // Show skeletons
  showSkeletons(tvSection, 12);
  showSkeletons(topSection, 12);
  const [shows, topRated, genres] = await Promise.all([
    getPopular('tv'),
    getTopRated('tv'),
    getGenres('tv'),
  ]);
  if (shows.length) {
    renderHero(hero, shows[0], 'tv');
  }
  // Render genre chips for TV
  if (genreContainer) {
    genreContainer.innerHTML = '';
    const allBtn = document.createElement('button');
    allBtn.textContent = 'All';
    ...
    allBtn.onclick = () => {
      ...
      allBtn.classList.add('bg-red-600', 'text-white');
      renderGrid(tvSection, shows.slice(1, 13), 'tv');
    };
    genreContainer.appendChild(allBtn);
    genres.forEach((g) => {
      const btn = document.createElement('button');
      ...
      btn.onclick = async () => {
        ...
        showSkeletons(tvSection, 12);
        try {
          const results = await discoverContent('tv', { with_genres: g.id });
          if (results.length) {
            renderHero(hero, results[0], 'tv');
            renderGrid(tvSection, results.slice(1, 13), 'tv');
          } else {
            tvSection.innerHTML = '<p class="text-gray-400">No shows found.</p>';
          }
        } catch (err) {
          console.error(err);
        }
  ...
  renderGrid(tvSection, shows.slice(1, 13), 'tv');
  renderGrid(topSection, topRated.slice(0, 12), 'tv');
  ...
  async function loadMoreTv() {
    if (tvState.loading || tvState.noMore) return;
    tvState.loading = true;
    try {
      let results = [];
      if (tvState.genreId) {
        const lang = getLanguage();
        const params = new URLSearchParams({ api_key: API_KEY, language: lang, page: tvState.page, with_genres: tvState.genreId });
        const data = await fetchJSON(`${API_BASE}/discover/tv?${params}`);
        results = data.results || [];
        if (data.total_pages && tvState.page >= data.total_pages) {
          tvState.noMore = true;
        }
      } else {
        results = await getPopular('tv', tvState.page);
        if (!results.length) {
          tvState.noMore = true;
        }
      }
      if (results.length) {
        appendGrid(tvSection, results, 'tv');
        tvState.page += 1;
      } else {
        tvState.noMore = true;
      }
  ...
  if (genreContainer) {
    genreContainer.querySelectorAll('button').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (active && active.textContent !== 'All') {
          const genreObj = genres.find((g) => g.name === active.textContent);
          tvState.genreId = genreObj ? genreObj.id : null;
        } else {
          tvState.genreId = null;
        }
        tvState.page = 2;
        tvState.noMore = false;
      });
    });
  }
}
...
async function initAnimePage() {
  const hero = document.getElementById('hero');
  const popularSec = document.getElementById('anime-popular');
  const topSec = document.getElementById('anime-top-rated');
  // Show skeletons
  showSkeletons(popularSec, 12);
  showSkeletons(topSec, 12);
  const shows = await getPopular('tv');
  // Filter anime: origin_country contains 'JP' or genre_ids includes 16
  const animeShows = shows.filter((show) => {
    return (
  ...
  const topAnime = topRated.filter((show) => {
    return (
      (show.origin_country && show.origin_country.includes('JP')) ||
      (show.genre_ids && show.genre_ids.includes(16))
    );
  ...
  if (animeShows.length) {
    renderHero(hero, animeShows[0], 'tv');
  }
  renderGrid(popularSec, animeShows.slice(1, 13), 'tv');
  renderGrid(topSec, topAnime.slice(0, 12), 'tv');
}
...
async function initDetailsPage() {
  ...
  try {
    ...
    // Populate download list with dummy links (placeholder as TMDB does not provide media files)
    ...
    ['480p', '720p', '1080p'].forEach((quality) => {
      ...
      link.textContent = `Download ${quality} (placeholder)`;
      link.onclick = (e) => {
        e.preventDefault();
        alert('Download functionality is a placeholder in this demo.');
      };
    ...
    function updateAddBtn() {
      const inList = isInWatchlist(id, type);
      addBtn.innerHTML = inList
        ? '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg><span>Remove from Watchlist</span>'
        : '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg><span>Add to Watchlist</span>';
    }
    updateAddBtn();
    addBtn.onclick = () => {
      const item = {
...
async function initWatchPage() {
  ...
  try {
    ...
    // Render server selection as a dropdown
    ...
    if (selectEl) {
      ...
      if (sources.length > 0) {
        ...
        selectEl.onchange = () => {
          ...
          player.innerHTML = `<iframe class="w-full h-80 md:h-[450px]" src="${url}" frameborder="0" allowfullscreen sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-presentation"></iframe>`;
        };
        // Auto select first option
        selectEl.selectedIndex = 0;
        player.innerHTML = `<iframe class="w-full h-80 md:h-[450px]" src="${sources[0].url}" frameborder="0" allowfullscreen sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-presentation"></iframe>`;
      } else {
        ...
        if (trailer) {
          player.innerHTML = `<iframe class="w-full h-80 md:h-[450px]" src="https://www.youtube.com/embed/${trailer.key}" title="${title} trailer" frameborder="0" allowfullscreen sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-presentation"></iframe>`;
        } else {
    ...
    downloadBtn.onclick = () => {
      ...
      if (type === 'movie') {
        downloadSources.push({ name: 'BunnyDDL', url: `https://bunnyddl.termsandconditionshere.workers.dev/movie/${id}` });
        downloadSources.push({ name: 'VidSrcDL', url: `https://dl.vidsrc.vip/movie/${id}` });
      } else {
        downloadSources.push({ name: 'BunnyDDL', url: `https://bunnyddl.termsandconditionshere.workers.dev/tv/${id}/${season}/${episode}` });
        downloadSources.push({ name: 'VidSrcDL', url: `https://dl.vidsrc.vip/tv/${id}/${season}/${episode}` });
      }
      downloadSources.forEach((src) => {
        ...
        link.className = 'block px-4 py-2 hover:bg-gray-700';
        link.textContent = `Download via ${src.name}`;
